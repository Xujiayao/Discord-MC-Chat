...
2. **自动生成凭证**: 若未绑定，Server 生成一个 6 位临时验证码（如 `A7X9P2`），Client 在游戏内发送包含内联可点击元素的消息指引玩家
   在 Discord 执行 `/link A7X9P2`，验证码 5 分钟有效。已绑定的玩家不会收到提醒。
...
3. **绑定账号**: 如果用户有绑定 Minecraft 玩家，则保底获得 OP 0 等级。
...
若 `user_mappings[].servers` 或 `role_mappings[].servers` 中没有某子服务器的对应条目，
则该子服务器自动使用该映射条目的顶层 `op_level` 作为回退值。
即：除非你需要为某子服务器单独调整 OP 等级，否则无需在 `servers` 中为其配置。
