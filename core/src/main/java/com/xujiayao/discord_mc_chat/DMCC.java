...
public final class DMCC {
	...
	public static boolean init() {
		try (ExecutorService executor = Executors.newSingleThreadExecutor(ExecutorServiceUtils.newThreadFactory("DMCC-Init"))) {
			return executor.submit(() -> {
				if (!I18nManager.loadInternalTranslationsOnly()) {
					// Should not happen! Not translated, as translations failed to load
				...
				if (System.console() == null && !IS_MINECRAFT_ENV) {
					// Likely started by double-clicking the JAR in a GUI; warn the user to start from the command line
				...
				// On failure: return false. Minecraft keeps running and the user can reload; standalone exits.

				boolean configs = !ModeManager.load()
						|| !ConfigManager.load()
						|| !I18nManager.load(ConfigManager.getString("language", I18nManager.detectLanguage()));
				...
				switch (ModeManager.getMode()) {
					case "single_server" -> {
						...
						// Server instance verifies the client with the secret; client authenticates with the same one
	...
	private static boolean shutdownInternal(boolean forReload) {
		try (ExecutorService executor = Executors.newSingleThreadExecutor(ExecutorServiceUtils.newThreadFactory("DMCC-Shutdown"))) {
			return executor.submit(() -> {
	...
	/**
	 * Re-initializes only if shutdown succeeded.
	 *
